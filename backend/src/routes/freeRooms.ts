...
// GET /api/rooms/available?date=YYYY-MM-DD&timeslot_ids[]=uuid
router.get('/available', async (req: Request, res: Response) => {
  const { date, timeslot_ids } = req.query;
  if (!date) return res.status(400).json({ error: 'date is required' });

  const tsIds = Array.isArray(timeslot_ids) ? timeslot_ids as string[] : timeslot_ids ? [timeslot_ids as string] : [];
     ...
     WHERE r.is_active = TRUE
       AND NOT EXISTS (
         ...
         WHERE rb.room_id = r.id
           AND rb.booking_date = $1::date
           AND ($2::uuid[] IS NULL OR rb.timeslot_id = ANY($2::uuid[]))
       )
       AND NOT EXISTS (
...
router.post('/book', async (req: Request, res: Response) => {
  ...
  const clash = await db.query(
    `SELECT 1 FROM room_bookings WHERE room_id=$1 AND booking_date=$2 AND timeslot_id=$3 LIMIT 1`,
    [room_id, booking_date, timeslot_id]
  );
  if ((clash.rowCount ?? 0) > 0) {
    return res.status(409).json({ error: 'Room is already booked for this timeslot' });
  }

  const result = await db.query(
    `INSERT INTO room_bookings (room_id, booked_by, booking_date, timeslot_id, label)
...
// GET /api/rooms/heatmap?week_start=YYYY-MM-DD
router.get('/heatmap', async (req: Request, res: Response) => {
  const result = await db.query(
    `SELECT r.building, te.timeslot_id, te.day,
       COUNT(te.id) AS occupied_rooms,
     ...
     WHERE r.is_active = TRUE
     GROUP BY r.building, te.timeslot_id, te.day
     ORDER BY r.building, te.day, te.timeslot_id`
  );
  res.json(result.rows);
});
