...
router.get('/', async (req: Request, res: Response) => {
  const result = await db.query(
    `SELECT * FROM blackout_dates
     ${req.query['term_id'] ? 'WHERE term_id = $1' : ''}
     ORDER BY date_start`,
...
router.post('/', requireRole('Super_Admin'), async (req: Request, res: Response) => {
  ...
  const d = parsed.data;
  if (d.scope === 'department' && !d.department_id) {
    return res.status(400).json({ error: 'department_id required for department-scoped blackout' });
  }

  const result = await db.query(
    `INSERT INTO blackout_dates (term_id, date_start, date_end, label, scope, department_id)
