...
router.get('/', async (req: Request, res: Response) => {
  const result = await db.query(
    `SELECT psg.*, json_agg(psm.batch_id) AS batch_ids
     FROM parallel_section_groups psg
     JOIN parallel_section_members psm ON psm.group_id = psg.id
     ${req.query['term_id'] ? 'WHERE psg.term_id = $1' : ''}
     GROUP BY psg.id`,
...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    for (const batchId of parsed.data.batch_ids) {
      await client.query(
        `INSERT INTO parallel_section_members (group_id, batch_id) VALUES ($1,$2)`,
