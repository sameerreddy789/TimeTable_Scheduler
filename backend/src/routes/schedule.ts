...
import { schedulingQueue } from '../queue/schedulingQueue';
...
// POST /api/schedule/run — pre-validate then enqueue job
router.post('/run', async (req: Request, res: Response) => {
  ...
  let report;
  try {
    report = await runPreValidation(client, term_id, department_id);
  } finally {
    client.release();
  }

  if (report.score < 100) {
    return res.status(422).json({ error: 'Pre-validation failed', report });
  }

  // Build job payload from DB
  const [rooms, faculty, subjects, batches, assignments, timeslots, parallelGroups, softWeights] =
    await Promise.all([
      ...
      db.query(`SELECT fp.*, COALESCE(array_agg(ftp.timeslot_id) FILTER (WHERE ftp.pref_type='not_available'), '{}') AS not_available_timeslots
                FROM faculty_profiles fp LEFT JOIN faculty_timeslot_prefs ftp ON ftp.faculty_id=fp.id
                GROUP BY fp.id`),
      db.query(`SELECT s.*, sdr.session_durations FROM subjects s LEFT JOIN session_duration_rules sdr ON sdr.subject_id=s.id
                WHERE s.department_id=$1`, [department_id]),
      ...
      db.query(`SELECT psg.*, json_agg(psm.batch_id) AS batch_ids
                FROM parallel_section_groups psg JOIN parallel_section_members psm ON psm.group_id=psg.id
                WHERE psg.term_id=$1 GROUP BY psg.id`, [term_id]),
      db.query(`SELECT constraint_key, weight FROM soft_constraint_weights WHERE term_id=$1`, [term_id]),
    ]);

  // Merge blocked slots (blackouts + bookings) per room
  const blackouts = await db.query(
    `SELECT * FROM blackout_dates WHERE term_id=$1`, [term_id]
  );
  const bookings = await db.query(
    `SELECT * FROM room_bookings WHERE booking_date >= CURRENT_DATE`
  );

  const blockedByRoom: Record<string, any[]> = {};
  for (const bd of blackouts.rows) {
    for (const room of rooms.rows) {
      blockedByRoom[room.id] = blockedByRoom[room.id] ?? [];
      // Add all timeslots on blackout days as blocked
  ...
  for (const bk of bookings.rows) {
    ...
    blockedByRoom[bk.room_id].push({
      ...
      timeslot_id: bk.timeslot_id,
      reason: 'booking',
    });
  }

  const softWeightsMap: Record<string, number> = {};
  for (const row of softWeights.rows) {
    softWeightsMap[row.constraint_key] = row.weight;
  }
  ...
  // Create scheduling_jobs record (staging)
  await db.query(
    `INSERT INTO scheduling_jobs (id, term_id, department_id, status, correlation_id)
...
// POST /api/schedule/:jobId/apply — promote selected option to timetable tables
router.post('/:jobId/apply', async (req: Request, res: Response) => {
  const { selected_option_index } = req.body;
  if (typeof selected_option_index !== 'number') {
    return res.status(400).json({ error: 'selected_option_index is required' });
  }

  const jobRow = await db.query(
    `SELECT * FROM scheduling_jobs WHERE id=$1 AND status='completed'`,
    [req.params['jobId']]
  );
  if (jobRow.rowCount === 0) {
    return res.status(404).json({ error: 'Job not found or not in completed state' });
  }

  const job = jobRow.rows[0];
  const options = job.solution_pool as any[];
  if (!options || selected_option_index >= options.length) {
  ...
  try {
    ...
    // Create timetable_version record
    ...
    // Batch INSERT all entries (single statement)
    const entries: any[] = selected.entries;
    if (entries.length > 0) {
      ...
      const flatValues = entries.flatMap(e => [
        versionId, e.batch_id, e.subject_id, e.faculty_id,
        e.room_id, e.timeslot_id, e.day, e.is_lab_block,
      ]);

      await pgClient.query(
        `INSERT INTO timetable_entries
           (timetable_version_id, batch_id, subject_id, faculty_id, room_id, timeslot_id, day, is_lab_block, version_number)
    ...
    // Mark job as applied
    await pgClient.query(
      `UPDATE scheduling_jobs SET status='applied', selected_option_index=$1, updated_at=NOW() WHERE id=$2`,
      [selected_option_index, job.id]
    );
...
// GET /api/schedule/:jobId/status
router.get('/:jobId/status', async (req: Request, res: Response) => {
  const result = await db.query(
    `SELECT id, status, error_detail,
       CASE WHEN solution_pool IS NOT NULL
         THEN jsonb_array_length(solution_pool)
         ELSE 0 END AS options_count
