...
// GET /api/analytics?term_id=&department_id=
router.get('/', async (req: Request, res: Response) => {
  ...
  // Find the active published timetable version for this dept+term
  const versionResult = await db.query(
    `SELECT id FROM timetable_versions
     WHERE term_id=$1 AND status='Published'
  ...
  if (versionResult.rowCount === 0) {
    return res.json({ message: 'No published timetable found', data: null });
  }
  ...
  const [roomUtil, facultyLoad, congestedDays, freePeriods] = await Promise.all([
    ...
    db.query(
      `SELECT r.id, r.room_number, r.building, r.room_type,
         ...
         ROUND(COUNT(te.id)::numeric /
           NULLIF((SELECT COUNT(*) FROM timeslots WHERE is_break=FALSE) * 5, 0) * 100, 2) AS utilization_pct
       FROM rooms r
       LEFT JOIN timetable_entries te ON te.room_id=r.id AND te.timetable_version_id=$1
       WHERE r.is_active=TRUE
       GROUP BY r.id ORDER BY utilization_pct DESC`,
      [versionId]
    ),
    ...
    db.query(
      `SELECT fp.id, fp.full_name, fp.max_classes_per_week,
       ...
       LEFT JOIN timetable_entries te ON te.faculty_id=fp.id AND te.timetable_version_id=$1
       GROUP BY fp.id ORDER BY load_pct DESC`,
      [versionId]
    ),

    // Top 3 most congested days
    db.query(
      `SELECT day, COUNT(*) AS simultaneous_classes
       FROM timetable_entries WHERE timetable_version_id=$1
       GROUP BY day ORDER BY simultaneous_classes DESC LIMIT 3`,
      [versionId]
    ),

    // Free periods per day across all batches
    db.query(
      `SELECT day,
