...
router.get('/', async (req: Request, res: Response) => {
  ...
  const result = await db.query(
    `SELECT fp.*,
     ...
     LEFT JOIN faculty_secondary_depts fsd ON fsd.faculty_id = fp.id
     LEFT JOIN faculty_timeslot_prefs ftp ON ftp.faculty_id = fp.id
     ${deptId ? 'WHERE fp.primary_department_id = $1' : ''}
     GROUP BY fp.id
...
router.post('/', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  ...
  // Validate max_classes_per_day <= total non-break timeslots
  const timeslotCount = await db.query(
    `SELECT COUNT(*) AS cnt FROM timeslots WHERE is_break = FALSE`
  );
  const totalSlots = parseInt(timeslotCount.rows[0].cnt, 10);
  if (d.max_classes_per_day > totalSlots) {
    return res.status(400).json({
      error: `max_classes_per_day (${d.max_classes_per_day}) exceeds total non-break timeslots (${totalSlots})`,
    });
  }
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO faculty_profiles (user_id, full_name, primary_department_id, max_classes_per_day,
         max_classes_per_week, avg_leave_days_per_month, consecutive_class_limit)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
      [d.user_id ?? null, d.full_name, d.primary_department_id, d.max_classes_per_day,
       d.max_classes_per_week, d.avg_leave_days_per_month ?? null, d.consecutive_class_limit]
    );
    const faculty = result.rows[0];

    if (d.secondary_depts.length > 0) {
      for (const sd of d.secondary_depts) {
        await client.query(
          `INSERT INTO faculty_secondary_depts (faculty_id, department_id, workload_pct) VALUES ($1,$2,$3)`,
          [faculty.id, sd.department_id, sd.workload_pct]
        );
      }
    }
    ...
    for (const tsId of d.preferred_timeslots) {
      await client.query(
        `INSERT INTO faculty_timeslot_prefs (faculty_id, timeslot_id, pref_type) VALUES ($1,$2,'preferred')`,
        [faculty.id, tsId]
      );
    }
    for (const tsId of d.not_available_timeslots) {
      await client.query(
        `INSERT INTO faculty_timeslot_prefs (faculty_id, timeslot_id, pref_type) VALUES ($1,$2,'not_available')`,
        [faculty.id, tsId]
      );
    }
...
router.patch('/:id', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (setClauses.length > 0) {
      ...
      const result = await client.query(
        `UPDATE faculty_profiles SET ${setClauses.join(', ')} WHERE id = $${idx} RETURNING *`,
        values
      );
    ...
    if (secondary_depts !== undefined) {
      ...
      for (const sd of secondary_depts) {
        await client.query(
          `INSERT INTO faculty_secondary_depts (faculty_id, department_id, workload_pct) VALUES ($1,$2,$3)`,
          [req.params['id'], sd.department_id, sd.workload_pct]
        );
      }
    }
