...
const ACADEMIC_YEARS = ['1st Year', '2nd Year', '3rd Year', '4th Year', 'PG Year 1', 'PG Year 2'] as const;
...
router.get('/', async (req: Request, res: Response) => {
  ...
  const result = await db.query(
    `SELECT b.*, d.name AS department_name, d.code AS department_code
     ...
     ${deptId ? 'WHERE b.department_id = $1' : ''}
     ORDER BY d.code, b.academic_year, b.section_label`,
    deptId ? [deptId] : []
  );
...
router.post('/', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  ...
  const user = (req as any).user;
  // Department_Admin can only create batches in their own department
  ...
  try {
    ...
    if (err.code === '23505') {
      return res.status(409).json({ error: 'Batch with this department, year, and section already exists' });
    }
    throw err;
  }
});
...
router.patch('/:id', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  ...
  const result = await db.query(
    `UPDATE batches SET
       ...
       student_strength = COALESCE($4, student_strength),
       max_weekly_classes = COALESCE($5, max_weekly_classes)
     WHERE id = $6 RETURNING *`,
    [department_id ?? null, academic_year ?? null, section_label ?? null,
     student_strength ?? null, max_weekly_classes ?? null, req.params['id']]
  );
