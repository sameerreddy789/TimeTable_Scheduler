...
const subjectSchema = z.object({
  ...
  // session_duration_rules: array of session durations that must sum to weekly_hours
  session_durations: z.array(z.number().int().positive()).min(1),
});

// GET /api/subjects?department_id=
router.get('/', async (req: Request, res: Response) => {
  const user = (req as any).user;
  const deptId = user.role === 'Department_Admin' ? user.department_id : req.query['department_id'];

  const result = await db.query(
    `SELECT s.*, sdr.session_durations
     FROM subjects s
     LEFT JOIN session_duration_rules sdr ON sdr.subject_id = s.id
     ${deptId ? 'WHERE s.department_id = $1' : ''}
     ORDER BY s.name`,
    deptId ? [deptId] : []
  );
...
router.post('/', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  ...
  // Validate: weekly_hours must equal sum of session_durations × classes_per_week
  const sessionSum = d.session_durations.reduce((a, b) => a + b, 0);
  if (sessionSum !== d.weekly_hours) {
    return res.status(400).json({
      error: `weekly_hours (${d.weekly_hours}) must equal sum of session_durations (${sessionSum})`,
    });
  }

  // Warn if Lab type without Lab room type (return 400 unless confirmed)
  if (d.subject_type === 'Lab' && d.preferred_room_type !== 'Lab') {
    if (!req.body.confirm_lab_room_mismatch) {
      return res.status(422).json({
        error: 'Lab subject should have preferred_room_type of Lab',
        code: 'LAB_ROOM_MISMATCH',
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO subjects (department_id, name, code, subject_type, weekly_hours, classes_per_week,
         preferred_room_type, required_room_features, fixed_timeslot_id, fixed_day)
    ...
    await client.query(
      `INSERT INTO session_duration_rules (subject_id, session_durations) VALUES ($1, $2)`,
      [subject.id, d.session_durations]
    );
...
router.delete('/:id', requireRole('Super_Admin', 'Department_Admin'), async (req: Request, res: Response) => {
  // Prevent deletion if referenced in a published timetable
  const inUse = await db.query(
    `SELECT 1 FROM timetable_entries te
     JOIN timetable_versions tv ON tv.id = te.timetable_version_id
     WHERE te.subject_id = $1 AND tv.status = 'Published' LIMIT 1`,
    [req.params['id']]
  );
  if ((inUse.rowCount ?? 0) > 0) {
    return res.status(409).json({ error: 'Cannot delete subject: referenced in a published timetable' });
  }
