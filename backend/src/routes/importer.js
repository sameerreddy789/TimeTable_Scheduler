...
router.post('/rooms', upload.single('file'), async (req, res) => {
  ...
  let rows;
  try { rows = parseSheet(wb, 'Rooms'); } catch (e) { return res.status(400).json({ error: e.message }); }
  ...
  try {
    ...
    for (const r of valid) {
      await client.query(
        `INSERT INTO rooms (room_number,room_type,capacity,building,features) VALUES ($1,$2,$3,$4,$5)
...
router.post('/faculty', upload.single('file'), async (req, res) => {
  ...
  const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
  let rows;
  try { rows = parseSheet(wb, 'Faculty'); } catch (e) { return res.status(400).json({ error: e.message }); }
  const errors = []; const valid = [];
  rows.forEach((r, i) => {
    const rowNum = i + 2;
  ...
  try {
    ...
    for (const r of valid) {
      await client.query(
        `INSERT INTO faculty_profiles (full_name,email,department_id,max_classes_per_day) VALUES ($1,$2,$3,$4)
...
router.post('/subjects', upload.single('file'), async (req, res) => {
  ...
  const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
  let rows;
  try { rows = parseSheet(wb, 'Subjects'); } catch (e) { return res.status(400).json({ error: e.message }); }
  const errors = []; const valid = [];
  rows.forEach((r, i) => {
    const rowNum = i + 2;
    if (!r.name) errors.push({ row: rowNum, field: 'name', message: 'Required' });
    if (!r.code) errors.push({ row: rowNum, field: 'code', message: 'Required' });
    const wh = parseInt(r.weekly_hours, 10); const cpw = parseInt(r.classes_per_week, 10); const sd = parseInt(r.session_duration, 10);
    if (isNaN(wh) || isNaN(cpw) || isNaN(sd)) errors.push({ row: rowNum, field: 'weekly_hours/classes_per_week/session_duration', message: 'Must be integers' });
    else if (wh !== cpw * sd) errors.push({ row: rowNum, field: 'weekly_hours', message: `Must equal classes_per_week × session_duration (${cpw * sd})` });
    if (errors.filter(e => e.row === rowNum).length === 0) valid.push(r);
  });
  if (errors.length > 0) return res.status(422).json({ errors });
  const client = await db.connect();
  try {
    ...
    for (const r of valid) {
      await client.query(
        `INSERT INTO subjects (name,code,subject_type,weekly_hours,classes_per_week,session_duration,department_id) VALUES ($1,$2,$3,$4,$5,$6,$7)
...
router.post('/batches', upload.single('file'), async (req, res) => {
  ...
  const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
  let rows;
  try { rows = parseSheet(wb, 'Batches'); } catch (e) { return res.status(400).json({ error: e.message }); }
  const errors = []; const valid = [];
  rows.forEach((r, i) => {
    const rowNum = i + 2;
    if (!r.department_id) errors.push({ row: rowNum, field: 'department_id', message: 'Required' });
  ...
  try {
    ...
    for (const r of valid) {
      await client.query(
        `INSERT INTO batches (department_id,academic_year,section_label,student_strength) VALUES ($1,$2,$3,$4)
...
router.get('/templates/:entity', authenticate, (req, res) => {
  const entity = req.params['entity'];
  const templates = {
    rooms: { sheet: 'Rooms', headers: ['room_number', 'room_type', 'capacity', 'building', 'features'] },
    faculty: { sheet: 'Faculty', headers: ['full_name', 'email', 'department_id', 'max_classes_per_day'] },
    subjects: { sheet: 'Subjects', headers: ['name', 'code', 'subject_type', 'weekly_hours', 'classes_per_week', 'session_duration', 'department_id'] },
    batches: { sheet: 'Batches', headers: ['department_id', 'academic_year', 'section_label', 'student_strength'] },
  };
