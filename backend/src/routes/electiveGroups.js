...
router.get('/', async (req, res) => {
  const result = await db.query(
    `SELECT eg.*, json_agg(egb.batch_id) AS batch_ids, SUM(b.student_strength) AS combined_strength
     FROM elective_groups eg JOIN elective_group_batches egb ON egb.group_id=eg.id
     JOIN batches b ON b.id=egb.batch_id
     ${req.query['term_id'] ? 'WHERE eg.term_id=$1' : ''} GROUP BY eg.id`,
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    for (const batchId of parsed.data.batch_ids) {
      await client.query(`INSERT INTO elective_group_batches (group_id,batch_id) VALUES ($1,$2)`, [group.rows[0].id, batchId]);
    }
