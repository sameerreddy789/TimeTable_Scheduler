...
export async function runPreValidation(
  ...
  await check('16.1a', 'Every subject assignment must have an assigned faculty member',
    '/subjects', async () => {
      const r = await client.query(
        `SELECT 1 FROM subject_assignments WHERE term_id=$1 AND faculty_id IS NULL LIMIT 1`,
        [termId]
      );
  ...
  await check('16.1b', 'Faculty max_classes_per_day must not exceed total non-break timeslots',
    '/faculty', async () => {
      ...
      const total = parseInt(slots.rows[0].cnt, 10);
      const r = await client.query(
        `SELECT 1 FROM faculty_profiles WHERE max_classes_per_day > $1 LIMIT 1`, [total]
      );
      return (r.rowCount ?? 0) === 0;
    });
  ...
  await check('16.1d', 'Total weekly class hours per batch must not exceed available timeslots per week',
    '/batches', async () => {
      ...
      const slotsPerWeek = slotsPerDay * 5; // Mon–Fri
      const r = await client.query(
        `SELECT 1 FROM (
         ...
         ) t WHERE t.total_classes > $2 LIMIT 1`,
        [termId, slotsPerWeek]
      );
      return (r.rowCount ?? 0) === 0;
    });
      ...
      const r = await client.query(
        `SELECT 1 FROM elective_groups eg
         JOIN (
           ...
           FROM elective_group_batches egb JOIN batches b ON b.id=egb.batch_id
           GROUP BY group_id
         ) cs ON cs.group_id=eg.id
         WHERE eg.term_id=$1
           AND NOT EXISTS (
             SELECT 1 FROM rooms r WHERE r.is_active=TRUE AND r.capacity >= cs.combined
           )
         LIMIT 1`,
        [termId]
      );
    ...
    '/faculty', async () => {
      const r = await client.query(
        `SELECT 1 FROM (
         ...
         ) t WHERE t.assigned * 1 > t.max_classes_per_week LIMIT 1`,
        [termId]
      );
      return (r.rowCount ?? 0) === 0;
    });
  ...
  await check('16.1h', 'No session duration rule can require a session longer than a single timeslot',
    '/subjects', async () => {
      // Get max timeslot duration in hours
      ...
      const r = await client.query(
        `SELECT 1 FROM session_duration_rules sdr
         WHERE EXISTS (
           SELECT 1 FROM unnest(sdr.session_durations) AS d WHERE d > $1
         ) LIMIT 1`,
        [maxDur]
      );
      return (r.rowCount ?? 0) === 0;
    ...
    '/faculty', async () => {
      const r = await client.query(
        `SELECT 1 FROM (
         ...
         ) t WHERE t.max_classes_per_day > t.consecutive_class_limit * 2 LIMIT 1`,
        [termId]
      );
      return (r.rowCount ?? 0) === 0;
    });
        ...
        `SELECT 1 FROM subjects s
         WHERE s.required_room_features IS NOT NULL AND array_length(s.required_room_features, 1) > 0
           AND NOT EXISTS (
             ...
             JOIN room_feature_tags rft ON rft.room_id=r.id
    ...
    '/batches', async () => {
      const r = await client.query(
        `SELECT 1 FROM (
         ...
         ) t WHERE t.total > t.max_weekly_classes LIMIT 1`,
        [termId]
      );
      return (r.rowCount ?? 0) === 0;
    });
